class Login3Controller < ApplicationController
   require 'digest/sha2'
  ...
  def login_page  
    ...
    username = params[:user_from_login3]
    pw_from_login = params[:pw_from_login3]
    ...
    if !username.blank?
      ...
      if pw_array.length > 0
        ...
        if pw_from_db == encoded_pw
          ...
          redirect_to :controller => '/sourcing_partners' 
        end
      end
    end
  end
  ...
  def logout
    ...
    redirect_to :controller => 'login3', :action => 'login_page'
