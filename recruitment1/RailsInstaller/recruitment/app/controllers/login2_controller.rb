class Login2Controller < ApplicationController
 require 'digest/sha2'
  ...
  def login_page  
    ...
    username = params[:user_from_login2]
    pw_from_login = params[:pw_from_login2]
    ...
    if !username.blank?
      ...
      if pw_array.length > 0
        ...
        if pw_from_db == encoded_pw
          ...
          redirect_to :controller => '/technologies' 
        end
      end
    end
  end
  ...
  def logout
    ...
    redirect_to :controller => 'login2', :action => 'login_page'
